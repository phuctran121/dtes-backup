...
export default function WhyCardItem({ card }: WhyCardItemProps) {
  return (
    <Card className="rounded-2xl border-0 shadow-[0_2px_10px_rgba(0,0,0,0.05)] bg-white relative hover:shadow-[0_6px_20px_rgba(0,0,0,0.08)] transition-shadow duration-300">
      <CardContent className="flex flex-col items-start justify-between h-42 sm:h-50 lg:h-60 py-2 text-left relative">
        <div className="flex flex-col">
          <h3 className="text-lg md:text-xl lg:text-2xl font-semibold mb-1 tracking-tight">
            {card.title}
          </h3>
          <p className="text-sub-title font-normal text-sm md:text-base lg:text-lg ">
            {card.description}
          </p>
        </div>
        <Image
